...
function main() {
  ...
  /**
   * 勝ち幅と負け幅、そこから出るコスト控除後のリスクリワード。
   *
   * 実測値が渡されていればそちらを使う。**的中率は決済条件とセットでしか
   * 意味を持たない。** 5日で切る決済で測った54%を1:2の損切り利確に
   * 当てはめると、勝ち幅が実際の倍近くになって期待値が跳ね上がる。
   *
   * 破産の計算に生のRRを渡すのも同じ間違いで、コストを払わない口座を
